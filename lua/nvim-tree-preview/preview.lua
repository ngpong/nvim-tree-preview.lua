...
function Preview:set_win_options()
  ...
  for optname, optval in pairs(config.wo) do
    vim.api.nvim_set_option_value(optname, optval, { win = win, scope = "local" })
  end
end
...
function Preview:get_win()
  ...
  local tree_witdh = vim.fn.winwidth(tree.win)
  local width_delta = (size.width + tree_witdh) - editor_width
  if width_delta >= -2 then
    size.width = size.width - math.abs(width_delta) - 5
  end
